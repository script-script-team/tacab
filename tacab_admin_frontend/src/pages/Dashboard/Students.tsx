...
import { useGetAllStudents, useSearchStudent } from '@/react-query/student.hooks'
...
function Students() {
  ...
  const ITSearchStudents = student?.students?.filter((std) => std.subject === 'IT')
  const ComputerSearchStudents = student?.students?.filter(
    (std) => std.subject === 'Computer'
  )

  const displayedITStudents = value && ITSearchStudents?.length ? ITSearchStudents : ITStudents;
const displayedComputerStudents = value && ComputerSearchStudents?.length ? ComputerSearchStudents : ComputerStudents;
    ...
    <div className='w-full flex flex-col gap-4 min-h-[83.5vh] rounded-lg bg-white dark:bg-gray-950 p-5'>
      <div className="w-full flex justify-between">
        <form onSubmit={(e) => e.preventDefault()} className="w-full">
                <Input onChange={(e) => SetValue(e.target.value)} className='xs:w-full sm:w-full md:w-[50%] lg:w-[50%] xl:w-[50%]' placeholder='Search by name or phone number' />
              </form>
              {!ITSearchStudents && <div className="flex gap-2">
              <Button className='cursor-pointer' disabled={isLoading || isFetching || page === 1} onClick={() => setPage(page - 1)}><ChevronLeft /></Button>
              <Button className='cursor-pointer' disabled={isLoading || isFetching || page >= data?.total!} onClick={() => setPage(page + 1)}><ChevronRight /></Button>
              </div>}
      </div>
        ...
        <div className='flex w-full flex-col gap-6'>
          <Tabs defaultValue='it'>
            ...
            <TabsContent value='computer'>
              <ComputerStudentTable students={displayedComputerStudents || []} />
            </TabsContent>
          </Tabs>
        </div>
      )}
