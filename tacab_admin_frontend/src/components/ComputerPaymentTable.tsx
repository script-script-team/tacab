...
import { useGetAllComputerPayments, useGetFee, useCompletePayment } from '@/react-query/payment.hooks'
...
const PaymentCell = ({ payment }: { payment?: IPayment }) => {
    ...
    onSubmit: (values) => {
      mutate({
        ...
        onSuccess: () => {
          ...
          formik.resetForm()
          client.invalidateQueries({ queryKey: ['all-computer-payments'] })
        }
      })
    },
  ...
  const feeFee = feeData?.fees.find((fee) => fee.subject === 'Computer')
...
const ComputerPaymentTable = ({ page }: { page: number }) => {
        ...
        {data?.payments?.map((pay) => (
          ...
          >
            ...
            {Array.from({ length: 3 }).map((_, index) => {
              ...
              return <PaymentCell key={index} payment={payment} />
            })}
          </TableRow>
        ))}
      </TableBody>
...
export default ComputerPaymentTable
